...
/// tabs in parallel so each section surfaces its own loading / empty /
/// error state independently.
final class CaseworkWorkspaceNotifierProvider
    extends
        $AsyncNotifierProvider<CaseworkWorkspaceNotifier, CaseworkSnapshot> {
  /// Owns the "Mis gestiones" workspace state (STORY-34). Loads the three
  /// tabs in parallel so each section surfaces its own loading / empty /
  /// error state independently.
...
abstract class _$CaseworkWorkspaceNotifier
    extends $AsyncNotifier<CaseworkSnapshot> {
  ...
  void runBuild() {
    final created = build();
    final ref =
        this.ref as $Ref<AsyncValue<CaseworkSnapshot>, CaseworkSnapshot>;
    final element =
        ref.element
            as $ClassProviderElement<
              AnyNotifier<AsyncValue<CaseworkSnapshot>, CaseworkSnapshot>,
              AsyncValue<CaseworkSnapshot>,
              Object?,
              Object?
            >;
